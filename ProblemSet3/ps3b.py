...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        """
        probOfReproduction = (self.getMaxBirthProb() * (1.0 - popDensity))
        if random.random() <= probOfReproduction:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    timeSteps = [0 for x in range(300)]
    ...
    averages = [(x / float(numTrials)) for x in timeSteps]
    pylab.plot(averages, label='Simulation Without Drug')
...
class ResistantVirus(SimpleVirus):
    ...
    def isResistantTo(self, drug):
        ...
        otherwise.
        """
        if not drug in self.getResistances():
            return False
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if resistantToAllDrugs == True:
            probOfReproduction = (self.getMaxBirthProb() * (1.0 - popDensity))
            if random.random() <= probOfReproduction:
                
                offspringResistances = self.getResistances().copy()
                for drug in offspringResistances.keys():
                    if offspringResistances[drug] == True:
        ...
        for virus in self.viruses:
            ...
            for drug in drugResist:
                if not virus.isResistantTo(drug):
                    resistant = False
            if resistant == True:
                numberResistantToAll += 1
        return numberResistantToAll
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    timeSteps = [0 for x in range(300)]
    drugResistantParticles = [0 for x in range(300)]
    ...
    for trial in range(numTrials):
        ...
        for t in range(len(timeSteps) / 2):
            timeSteps[t] += patient.update()
            drugResistantParticles[t] += patient.getResistPop(['guttagonol'])
        
        patient.addPrescription('guttagonol')
        
        for t in range(len(timeSteps) / 2, len(timeSteps)):
            timeSteps[t] += patient.update()
            drugResistantParticles[t] += patient.getResistPop(['guttagonol'])

    averages = [(x / float(numTrials)) for x in timeSteps]
    pylab.plot(averages, label='Ave total virus population')
    averages = [(x / float(numTrials)) for x in drugResistantParticles]
    pylab.plot(averages, label='Ave drug resistant particles')
